// src/pages/DetallePostulacion.jsx
    ...
    historial: [
      {
        id: "4181-JI-01",
        documentos: 3,
        fechaEnvio: "18/03/2025",
        estado: "Revisado con correcciones",
        comentarios: "Faltó el acta de pregrado",
        fechaRevision: "19/03/2025",
      },
    ],
  },
];
...
const getTipoVinculacionValue = (vinculacion) => {
  const map = {
    Estudiante: "estudiantes",
    "Asistente Graduado": "asistente_graduado",
    "Contrato OPS": "contrato_ops",
  };
  return map[vinculacion] || "";
};

const getSubvinculacionValue = (contrato, vinculacion) => {
  if (vinculacion === "Estudiante") {
    return contrato.includes("Pregrado")
      ? "Auxiliar de Pregrado"
      : "Auxiliar de Posgrado";
  }
  if (vinculacion === "Asistente Graduado") {
    return contrato.includes("Renovación")
      ? "Proceso de Renovación"
      : "Proceso Nuevo";
  }
  return contrato.includes("Investigación")
...
const DetallePostulacion = () => {
  ...
  const postulacion = mockPostulaciones.find((p) => p.codigo === codigo);

  if (!postulacion) return <p>No se encontró la postulación.</p>;

  return (
    <Header>
      <header className="detalle-header">
        <h3>Proyecto {postulacion.codigo}</h3>
        <h1>Documentos – {postulacion.nombre}</h1>
      </header>
          ...
          <div className="info-card">
            <div className="info">
              ...
              <p>
                Código del proyecto
                <br />
                <strong>{postulacion.codigo}</strong>
              </p>
            </div>
            <div className="info">
              <svg
              ...
              <p>
                Vinculación
                <br />
                <strong>{postulacion.vinculacion}</strong>
              </p>
            </div>
            <div className="info">
              <svg
              ...
              <p>
                Tipo de contrato
                <br />
                <strong>{postulacion.contrato}</strong>
              </p>
            </div>
            <div className="info">
              <svg
        ...
        <div className="historial-section">
          ...
          {open && (
            <Modal onClose={() => setOpen(false)}>
              <SubidaDocumentos
                codigoProyecto={postulacion.codigo}
                nombrePostulante={postulacion.nombre}
                tipoVinculacion={getTipoVinculacionValue(
                  postulacion.vinculacion
                )}
                subvinculacion={getSubvinculacionValue(
          ...
          <table>
            ...
            <tbody>
              {postulacion.historial.map((item, index) => (
                <tr key={index}>
                  <td>{item.id}</td>
                  <td>({item.documentos})</td>
                  <td>{item.fechaEnvio}</td>
                  <td className={`${postulacion.estado.replace(" ", "-")}`}>
                    {item.estado}
                  </td>
